...
if __name__ == "__main__":
    ...
    copy_dataset = CopyDataset(task_params)
    copy_loader = torch.utils.data.DataLoader(copy_dataset, batch_size=1, shuffle=True)
    ...
    for j in range(steps):
        ...
        for i in range(data['input'].shape[0]):
            print(f"Input: {data['input'][i], data['input'][i].shape, data['input'][i].unsqueeze(0).shape}")
            print(f"Target: {data['target'][i]}")
            print()
